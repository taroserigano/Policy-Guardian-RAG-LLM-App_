...
Runs automatically and generates results
"""
import requests
import json
import sys
...
try:
    ...
    print(f"  llama3.1:8b: {'✅' if has_base else '❌ NOT FOUND'}")
    print(f"  policy-compliance-llm: {'✅' if has_ft else '❌ NOT FOUND'}")
...
for i, test in enumerate(tests, 1):
    ...
    # Test base model
    print(f"\n📦 Testing base model (llama3.1:8b)...", end="", flush=True)
    ...
    if base_answer:
        ...
        print(f"   Answer: {base_answer[:150]}...")
    else:
        base_score = 0
    ...
    print(f"\n✨ Testing fine-tuned model (policy-compliance-llm)...", end="", flush=True)
    ...
    if improvement > 0:
        print(f"\n   ✅ Fine-tuned is MORE accurate (+{improvement} keywords)")
    elif improvement == 0:
        print(f"\n   ⚖️  Both models equally accurate")
    else:
        print(f"\n   ❌ Base model was more accurate")
...
if improvement_pct > 20:
    ...
    print(f"   ✓ MINOR IMPROVEMENT (+{improvement_pct:.0f}%)")
else:
    print(f"   ⚠ NO IMPROVEMENT - Consider more training")
